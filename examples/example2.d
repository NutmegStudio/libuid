...
void main() {
    auto win = new Window("libuid Histogram Example", 640, 480, 1).setMargined(true)
    ...
    foreach (i; 0 .. 10) {
        ...
        auto spinbox = new Spinbox(0, 100).setValue(uniform(0, 101)).addOnChanged(delegate(Spinbox) {
    ...
    colorButton.setColor(brush.r, brush.g, brush.b, brush.a).addOnChanged(delegate(ColorButton) {
        histogram.queueRedrawAll;
    });
    ...
    histogram.addDraw(delegate(Area a, DrawParams p) {
        ...
        path.addRectangle(0, 0, p.areaWidth, p.areaHeight).end;
        ...
        path.newFigure(xoffLeft, yoffTop).lineTo(xoffLeft, yoffTop + gs.height)
            .lineTo(xoffLeft + gs.width, yoffTop + gs.height).end;
        ...
        auto constructGraph(double width, double height, bool extend) {
            ...
            if (extend) {
                path.lineTo(width, height).lineTo(0, height).closeFigure;
            }
            path.end;
            return path;
        }
        ...
        if (currentPoint != -1) {
            ...
            path = Path(FillMode.Winding).newFigureWithArc(loc.xs[currentPoint],
                loc.ys[currentPoint], pointRadius, 0, 6.23, 0).end;
            p.context.fill(path, brush);
        }
        ...
        bool inPoint(double x, double y, double xtest, double ytest) {
            x -= xoffLeft;
            y -= yoffTop;
            return (x >= xtest - pointRadius) && (x <= xtest + pointRadius)
                && (y >= ytest - pointRadius) && (y <= ytest + pointRadius);
        }
        ...
        currentPoint = i;
        histogram.queueRedrawAll;
    }).addMouseCrossed(delegate(Area a, int left) {}).addDragBroken(delegate(Area a) {
